...
(function() {
	...
	var Launchie = function() {
		...
		var htmlRegex = /(([a-z]+:\/\/)?(([a-z0-9\-]+\.)+([a-z]{2}|aero|arpa|biz|com|coop|edu|gov|info|int|jobs|mil|museum|name|nato|net|org|pro|travel|local|internal))(:[0-9]{1,5})?(\/[a-z0-9_\-\.~]+)*(\/([a-z0-9_\-\.]*)(\?[a-z0-9+_\-\.%=&amp;]*)?)?(#[a-zA-Z0-9!$&'()*+.=-_~:@/?]*)?)(\s+|$)/gi;

		this.launchApp = function(phrase, context, output) {
			if(htmlRegex.test(phrase))
				spawner.exec('"' + library["chrome"] + '" ' + phrase[1]);
			else
				spawner.execFile(library[phrase[1].toLowerCase()]);
		}
	}
